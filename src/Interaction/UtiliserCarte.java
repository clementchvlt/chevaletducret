...
public class UtiliserCarte extends Interaction{
	...
	public void Traiter(Object o) {
		...
		try {
			...
			if(cible == 1) {
				obj=plateau.getAdversaire(plateau.getJoueurCourant()).getHeros();
				plateau.getJoueurCourant().utiliserCarte(carte, obj);
			}else if(cible == 2) {
				System.out.println("Quelle carte tu veux attaquer (donne un boout de son nom\n");
				choixCarte2= console.readLine();
				try {
					carte2=plateau.getAdversaire(plateau.getJoueurCourant()).getCarteEnJeu(choixCarte2);
					
					plateau.getJoueurCourant().utiliserCarte(carte, carte2);
